...
(function () {
  ...
  function captureHeaders(headers) {
    ...
    headersReady = true;
    console.log('Twitter Location Extension: Captured Twitter API headers:', Object.keys(headerObj));
  }
  ...
  window.fetch = function (...args) {
    ...
    if (typeof url === 'string' && url.includes('x.com/i/api/graphql')) {
      if (options.headers) {
        ...
        console.log('Twitter Location Extension: Captured Twitter headers:', Object.keys(twitterHeaders || {}));
      }
    }
  ...
  setTimeout(() => {
    if (!headersReady) {
      console.log('Twitter Location Extension: No Twitter headers captured yet, using defaults');
      twitterHeaders = {
  ...
  window.addEventListener('message', async function (event) {
    ...
    if (event.data && event.data.type === '__fetchTwitterLocation') {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(`Twitter Location Extension: API response for ${screenName}:`, data);
          location = data?.data?.user_result_by_screen_name?.result?.about_profile?.account_based_in || null;
          console.log(`Twitter Location Extension: Extracted location for ${screenName}:`, location);

          // Debug: log the full path to see what's available
          if (!location && data?.data?.user_result_by_screen_name?.result) {
            console.log('Twitter Location Extension: User result available but no location:', {
          ...
          if (response.status === 429) {
            ...
            if (resetTime) {
              ...
              console.log(`Twitter Location Extension: Rate limited! Limit: ${limit}, Remaining: ${remaining}`);
              console.log(`Twitter Location Extension: Rate limit resets at: ${resetDate.toLocaleString()}`);
              console.log(
                `Twitter Location Extension: Waiting ${Math.ceil(waitTime / 1000 / 60)} minutes before retrying...`,
              );

              // Store rate limit info for content script
              window.postMessage(
                {
                  type: '__twitterRateLimitInfo',
            ...
            console.log(
              `Twitter Location Extension: Twitter API error for ${screenName}:`,
              response.status,
              response.statusText,
        ...
        window.postMessage(
          {
            type: '__twitterLocationResponse',
            screenName,
            location,
            requestId,
        ...
        console.error('Twitter Location Extension: Error fetching location:', error);
        window.postMessage(
          {
            type: '__twitterLocationResponse',
            screenName,
